...
[vaast-image] (https://github.com/jspsych/jspsych-contrib/blob/main/packages/plugin-vaast-image/README.md) | [Cédric Batailler](https://github.com/cedricbatailler) | This plugin displays an image stimulus on a background image, that the participant has to either approach or avoid, depending on the instructions. The scene changes accordingly. Used in the VAAST (Visual Approach and Avoidance by the Self Task).
