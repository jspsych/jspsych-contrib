...
class RokPlugin implements JsPsychPlugin<Info> {
    ...
    trial(display_element: HTMLElement, trial: TrialType<Info>) {
        ...
        var stimulus_keyframe_time = assignParameterValue(trial.stimulus_keyframe_time, .1);//info.parameters.stimulus_keyframe_time.default);
        ...
        var units = assignParameterValue(trial.units, null);//info.parameters.units.default);
        ...
        for (let i = 0; i < nApertures; i++) {
            ...
            for (let j = 0; j < nOob; j++) {
                ...
                if (stimulusType === 0) {
                    oob = new Oob(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i], units);
                } else if (stimulusType === 1) {
                    oob = new OobCircle(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i], units);
                } else if (stimulusType === 2) {
                    oob = new OobSquare(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i], units);
                } else if (stimulusType === 3) {
                    oob = new OobBird(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, canvasArray[i], ctxArray[i], units);
                    ...
                    oob = new OobImage(size, oobColor, orientation, movementDirection, speed, speedRandomisation, apertureType, randomWalk, randomOrient, isFade, imageArray, keyframes, keyframeTime, mirrorType, canvasArray[i], ctxArray[i], units);
                }
                oobs.push(oob);
            }
...
class Oob {
    ...
    constructor(size, color, orientation, movementDirection,
                speed, randomisation, apertureType, isRandomWalk,
                isRandomOrientated, isFade, canvas, ctx,
                units,) {
        this.canvas = canvas;
        if (units === 'px') {
