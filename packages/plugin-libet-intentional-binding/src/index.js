  ...
  const info = {
    ...
    parameters: {
      cond: {
        ...
        description: 'Specifies whether condition ("baseline-key", "baseline-tone", "operant-key", or "operant-tone".'
      },
      tone_file: {
      ...
      offset_range: {
        ...
        description: 'When the participant is able to move the clock hand, its initial angle will be offset from the correct angle by an absolute amount drawn from this range (in radians). Half the time the initial angle will be offset a positive amount, half the time a negative amount.'
      },
      fixation_ms: {
        type: jspsych.ParameterType.INT,
      ...
      early_ms: {
        ...
        default: 2560,
        description: 'The earliest allowable keypress, measured in milliseconds since the clock hand begins rotating.'
      },
      early_fcn: {
  ...
  class jsPsychLibetIntentionalBindingPlugin {
    ...
    trial(display_element, trial) {
      ...
      function ctrl_fcn(ctrl) {
        ...
        // of the trial
        if (ctrl == 'start trial') { // begin a new trial
