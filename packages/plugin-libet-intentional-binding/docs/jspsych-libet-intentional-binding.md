...
| est_wo_button_press      | boolean          | false                | If `true`, and if `hand_est` is true, the participant can make an estimate even if they did not make a keypress during the trial. |
...
| instructions_wo_button_press             | HTML string      | undefined                   | The instructions shown to the participant during estimation if they did not make a keypress. E.g., "When did you feel the urge to make a keypress?". Only applicable if `hand_est` and `est_wo_button_press` are set to `true`. If left undefined, the parameter takes on the same value as `instructions`. |
