...
class meyeExtension implements JsPsychExtension {
	...
	constructor(private jsPsych: JsPsych) {
		...
		this.updatePrediction = (delay) => {
			...
			if (this.video.readyState >= 2) this.updatePredictionTimeout = setTimeout(this.predictOnce, delay);
		}
		...
		this.predictFrame = () => {
			...
			var frame: any = tf.browser.fromPixels(this.video, 3)
				.slice([y, x], [s, s])
				.resizeBilinear([128, 128])
				.mul(tf.tensor1d([0.2989, 0.587, 0.114])).sum(2);
		...
		this.updatePupilLocator = (x, y) => {
			if (x < 0 || y < 0) this.pupilXLocator.style.display = this.pupilYLocator.style.display = 'none';
			else {
		...
		this.updateMode = (mode) => { this.mode = mode; }
	...
	on_start = (params: any): void => {
		...
		let obj = JSON.parse(this.jsPsych.data.get().filter({trial_type: "meye-calibration"}).json());
		...
		if (typeof params != 'undefined') {
			...
			if (typeof params.detection_interval != 'undefined') {
				...
				this.beginInterval = setInterval(this.snapshotHandler, params.detection_interval, null, 'interval detection');
			}
		}
	};
	...
	on_finish = ({}: OnFinishParameters): any => {
		...
		return this.snapshotHandler("Trial", "finished").then( (test) => {
			...
			this.pluginPupilData[this.pluginPupilData.length - 2] = this.pluginPupilData[this.pluginPupilData.length - 1];
