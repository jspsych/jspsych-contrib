...
const info = <const>{
  ...
  data: {
    /**
     * An object containing the data for each question.
     * `name:` The name for each question. If the `name` parameter is defined for the question,
     * then the response object will use the value of `name` as the key for each question.
     * Otherwise, the first question in the trial is recorded as `Q1`, the second as `Q2`, and so on.
     * `item_pos:` The position of the question on the page (from the top).
     * `resp_pos:` The position of the response selected on the Likert scale for that question, encoded as an integer.
     * `response:` The response to the question, encoded as an integer. Will be identical to `resp_pos`
     * if `reverse = false` and `zero_indexed = true`.
     */
    responses: {
      type: ParameterType.COMPLEX,
    },
    /**
     * An object containing a record of all radio button events on the page.
     * `event_target`: The target of the radio button event. The naming convention of each radio button is `{name}_{resp_pos}`.
     * `event_time`: The time of the event in milliseconds. The time is measured from when the questions first appear on the screen.
     */
    page_events: {
      type: ParameterType.COMPLEX,
    },
    /**
     * An object containing diagnostics of the quality of responses.
     * `page_time:` The response time, in milliseconds, for the participant to make a response. The time is measured
     * from when the questions first appear on the screen until the participant's response(s) are submitted.
     * `honeypot:` The number of hidden radio buttons checked.
     * `straightlining`: The maximum fraction of responses of the same position.
     * `zigzagging:` The fraction of responses consistent with a zig-zagging response pattern.
     */
    diagnostics: {
      type: ParameterType.COMPLEX,
    },
...
class SurveyGridPlugin {
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    html += `<input id="jspsych-survey-grid-next" type="submit" value="${trial.button_label}"></input>`;
    html += "</div>";
